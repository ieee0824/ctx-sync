...
use crate::cli::{DecisionAddArgs, DecisionCommand, DecisionListArgs, DecisionSupersedeArgs};
...
pub fn run(command: &DecisionCommand) -> Result<()> {
    match command {
        ...
        DecisionCommand::Supersede(args) => supersede(args),
    }
}
...
fn supersede(args: &DecisionSupersedeArgs) -> Result<()> {
    let rt = Runtime::from_env()?;
    let ws = Workspace::open(&rt, &std::env::current_dir()?)?;
    ...
    let outcome = hint_on_conflict(ops::supersede_decision(
        &ws,
        &args.old,
        input,
        args.sync,
